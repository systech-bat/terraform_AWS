...
resource "aws_cloudfront_distribution" "bazcorp_cdn01" {
  ...
  default_cache_behavior {
   ...
   logging_config {
    ...
    bucket          = aws_s3_bucket.bazcorp_cf_logs.bucket_domain_name
    prefix          = "cf-logs/"
  }
  ...
  depends_on = [aws_s3_bucket.bazcorp_cf_logs]
}
...
resource "aws_s3_bucket" "bazcorp_cf_logs" {
  bucket = "bazcorp-cf-logs"
  acl = "private"
  ...
  tags = {
    Name        = "bazcorp-cf-logs"
    Environment = "Dev"
  }
}

resource "aws_s3_bucket_ownership_controls" "bazcorp_cf_logs_ownership" {
  bucket = aws_s3_bucket.bazcorp_cf_logs.bucket
